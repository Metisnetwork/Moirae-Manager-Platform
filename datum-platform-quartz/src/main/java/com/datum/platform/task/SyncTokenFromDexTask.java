...
//                tokenAddress.compareTo(uniswapV2FactoryDao.WETH()) < 0 ? tuple3.getValue1() : tuple3.getValue2()));
    ...
    private String getPrice(BigInteger wEth, BigInteger token) {
        ...
        BigDecimal numerator = new BigDecimal(wEth).multiply(inputAmount).multiply(feesD);
        BigDecimal denominator = new BigDecimal(token).subtract(inputAmount).multiply(feesN);
