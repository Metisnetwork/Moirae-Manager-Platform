...
CREATE TABLE `t_algorithm` (
  ...
  `max_equal_flag` bigint(20) DEFAULT NULL COMMENT '是否等于协同方最大数量：1:等于, 0:不等于',
  `min_numbers` bigint(20) DEFAULT NULL COMMENT '支持协同方最小数量',
  `min_equal_flag` bigint(20) DEFAULT NULL COMMENT '是否等于协同方最小数量: 1:等于, 0:不等于',
...
CREATE TABLE `t_workflow_node_output` (
  ...
  `save_partner_flag` tinyint(4) DEFAULT NULL COMMENT '是否保存至协同方: 1-是, 0-否',
...
CREATE TABLE `t_job` (
  ...
  `workflow_id` bigint(20) DEFAULT NULL COMMENT '工作流id',
  `job_name` varchar(30) DEFAULT NULL COMMENT '作业名称',
  `job_desc` varchar(200) DEFAULT NULL COMMENT '作业描述',
  ...
  `job_status` tinyint(4) DEFAULT NULL DEFAULT 1 COMMENT '作业状态：0-已关闭, 1-已开启',
  `status` tinyint(4) NOT NULL DEFAULT 1 COMMENT '状态: 0-无效，1- 有效',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
